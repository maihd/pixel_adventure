...
/* Next power of two */
__forceinline int Json_nextPOT(int x)
{
    x  = x - 1;
    x |= x >> 1;
    x |= x >> 2;
...
__forceinline void* JsonArray_grow(void* array, int reqsize, int elemsize, JsonAllocator* allocator)
{
    assert(elemsize > 0);
...
__forceinline void* JsonTempArray_toBufferFunc(void* buffer, int count, void* dynamicBuffer, int itemSize, JsonAllocator* allocator)
{
...
/* @funcdef: Json_Parse */
Json* Json_Parse(const char* json, int32_t jsonLength, JsonFlags flags)
{
...
Json* Json_ParseEx(const char* json, int32_t jsonLength, JsonAllocator allocator, JsonFlags flags)
{
...
/* @funcdef: Json_Find */
const Json* Json_Find(const Json* obj, const char* name)
{
